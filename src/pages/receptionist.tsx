...
export default function ReceptionistPage() {
  ...
  const getSurname = (booking?: BookingApi) => {
    ...
    return booking.guestUsername || "-";
  };

  const orderedRooms = useMemo(() => {
    return [...rooms]
      .sort((a, b) => a.roomNumber.localeCompare(b.roomNumber, "it"))
      .slice(0, maxRooms);
  }, [rooms]);
  ...
  const printReceipt = (payload: {
    ...
    const bookingLine = payload.booking
      ...
      : "";
    const guestLine = payload.booking
      ? payload.booking.guestFullName || payload.booking.guestUsername || ""
      : "";
    win.document.write(`
      <html>
        <head>
  ...
  const handleUpdateBooking = async () => {
    ...
    try {
      ...
      setToastMessage("Prenotazione aggiornata");
  ...
  const handleAddDeposit = async () => {
    ...
    try {
      ...
      setDepositPaymentMethodId("");
      setDepositNotes("");
      setToastMessage("Acconto inserito");
      window.setTimeout(() => setToastMessage(null), 2500);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="sticky top-0 z-20 rounded-md border border-default-200 bg-default-50 px-2 py-2 shadow-sm">
      ...
      {viewMode === "grid" ? (
        <Card>
          <CardHeader className="flex items-center justify-between">
            ...
            <Button size="sm" variant="flat" onClick={() => setViewMode("calendar")}>
              Apri calendario
            </Button>
          </CardHeader>
          <CardBody>
            <div className="grid grid-cols-7 gap-2">
              {orderedRooms.map((room) => {
                ...
                return (
                  <div
                    key={room.id}
                    className={`flex flex-col gap-1 rounded-md border px-2 py-2 text-xs ${statusClass}`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                      <span>Adulti {booking?.adults ?? 0}</span>
                      <span>Bambini {booking?.children ?? 0}</span>
                      <span>Infant {booking?.infants ?? 0}</span>
                    </div>
                    ...
                    {detailsTab === "guests" && (
                      <div className="grid gap-3 text-sm md:grid-cols-3">
                        ...
                        <div className="md:col-span-3 text-xs text-default-500">
                          Cliente: {selectedBooking.guestFullName || selectedBooking.guestUsername || "-"}
                        </div>
                      </div>
                    )}
      ...
      {contextMenu && (
        <div
          className="fixed z-50 min-w-[180px] rounded-md border border-default-200 bg-white p-1 text-sm shadow-lg"
      ...
      {toastMessage && (
        <div className="fixed bottom-4 left-1/2 z-[60] -translate-x-1/2 rounded-md border border-danger-200 bg-danger-50 px-4 py-2 text-sm font-medium text-danger-700 shadow-lg">
          {toastMessage}
        </div>
      )}
